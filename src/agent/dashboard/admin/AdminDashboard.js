import {
  Button, Card, CircularProgress, Divider, FormControl, FormLabel,
  Grid, Input, Sheet, Table, Typography,
} from '@mui/joy';
  ...
  return (
    ...
    >
      ...
      <Grid xs={4} style={gridItemStyle}>
        <Card style={cardItemStyle}>
          <AgentList
            endpoint="/api/admin/all_registered_users"
      ...
      <Grid xs={4} style={gridItemStyle}>
        <Card style={cardItemStyle}>
          <AgentList
            endpoint="/api/admin/all_nonregistered_users"
            title="Non-Registered Agents"
      ...
      <Grid xs={4} style={gridItemStyle}>
        <Card style={cardItemStyle}>
          <AgentList
            endpoint="/api/admin/all_deleted_users"
...
function AgentList({ endpoint, title, desc }) {
  ...
  const getUsers = () => {
    setIsLoading(true);
    setErrMsg('');
    authorizedAxios.get(endpoint).then((res) => {
      setUsers(res.data);
    }).catch(() => {
  ...
  return (
    <div>
      ...
      <Typography color="neutral" fontSize="sm">{desc}</Typography>
      ...
      <div>
        ...
        <Sheet
          variant="soft"
          sx={{
            height: '170px', mt: 1, overflow: 'auto', borderRadius: 4,
          }}
        >
  ...
  return (
    <div>
      ...
      <Typography color="neutral" fontSize="sm">
        Set an agent to &quot;unregistered&quot; and reset their password.
      </Typography>
      <Divider sx={{ mt: '10px', mb: '10px' }} />
      {isLoading && (
      <div style={{
      ...
      {!isLoading && (
      <form onSubmit={(e) => { e.preventDefault(); unregisterUser(username); }}>
        <FormControl>
        ...
        <FormControl>
          ...
          <Input
            ...
            value={password}
            style={{ color: 'var(--joy-palette-neutral-200)' }}
            endDecorator={(
              <Button
                color="neutral"
...
function DeleteAgentForm() {
  ...
  return (
    <div>
      ...
      <Typography color="neutral" fontSize="sm">
        Delete an agent or admins account. This is currently irreversible.
      </Typography>
      <Divider sx={{ mt: '10px', mb: '10px' }} />
      {isLoading && (
      <div style={{
      ...
      {!isLoading && (
      <form onSubmit={(e) => { e.preventDefault(); deleteUser(username); }}>
        <FormControl>
          <FormLabel>Username</FormLabel>
          <Input
            slotProps={COMMON_INPUT_MAX_LENGTH_SLOT_PROP}
...
function NewAgentForm() {
  ...
  return (
    <div>
      ...
      <Typography color="neutral" fontSize="sm">
        Create a new agent with a random password.
        They will then set their own password during registration
      ...
      {!isLoading && (
      <form onSubmit={(e) => { e.preventDefault(); registerUser(username, firstName, lastName); }}>
        <FormControl>
          <FormLabel>Username</FormLabel>
          <Input
            slotProps={COMMON_INPUT_MAX_LENGTH_SLOT_PROP}
